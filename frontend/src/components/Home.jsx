      ...
      <main className=" m-4 lg:m-8">
        <div className="flex flex-col gap-3 lg:gap-0 lg:flex-row lg:justify-between ">
          <span className="flex text-lg lg:text-xl items-center font-bold justify-center gap-2">
            <Star className="hidden lg:block fill-current text-black opacity-70" size={20} />
            <Star className="block lg:hidden fill-current text-black opacity-70" size={18} />
            <h1 className="tracking-widest opacity-70 uppercase"> Projects</h1>
          </span>
        ...
        <div className="pb-[2rem] mt-8 grid grid-cols-[repeat(auto-fill,minmax(350px,1fr))] gap-[2.5rem] mb-28 lg:mb-0">
          {/* project */}
          <ShowProjects projects={projectsData.filter(project=>project.categories.includes(selectedCategory))}/>
        </div>
      </main>
    </div>
